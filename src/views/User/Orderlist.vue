...
const statusConfig = {
  ...
  Missing: { label: 'สินค้ามาไม่ครบ', color: '#ef4444', bg: '#fdeded' },
  Delayed: { label: 'รอของเข้า', color: '#d97706', bg: '#fff7ed' },
...
// ── ORDER-TYPE FILTER (พร้อมส่ง / พรีออเดอร์) ผูกกับ path ของตัวเอง ──
const orderTypeTabs = [
  { key: 'all', label: 'ทั้งหมด', path: '/order-list' },
  { key: 'ready', label: 'พร้อมส่ง', path: '/order-list/ready-to-ship' },
  { key: 'preorder', label: 'พรีออเดอร์', path: '/order-list/preorder' },
]
const activeOrderTypeTab = ref(route.meta?.orderType ?? 'all')

watch(
  () => route.meta?.orderType,
  (orderType) => {
    activeOrderTypeTab.value = orderType ?? 'all'
  },
)

function setOrderTypeTab(tab) {
  if (tab.key === activeOrderTypeTab.value) return
  router.push(tab.path)
}

function matchOrderType(order, typeKey) {
  const normalized = String(order.Order_type || '').trim().toLowerCase()
  if (typeKey === 'ready') return normalized === 'ready'
  if (typeKey === 'preorder') return normalized === 'preorder' || normalized === 'pending_import'
  return true
}

const ordersByType = computed(() => {
  if (activeOrderTypeTab.value === 'all') return orders.value
  return orders.value.filter((o) => matchOrderType(o, activeOrderTypeTab.value))
})

// ── UI-ONLY: STATUS FILTER TABS ──
...
const filteredOrders = computed(() => {
  if (activeStatusTab.value === 'all') return ordersByType.value
  return ordersByType.value.filter((o) => matchTab(o, activeStatusTab.value))
})

const orderTypeCounts = computed(() => ({
  all: orders.value.length,
  ready: orders.value.filter((o) => matchOrderType(o, 'ready')).length,
  preorder: orders.value.filter((o) => matchOrderType(o, 'preorder')).length,
}))

// ── UI-ONLY: PROGRESS STEP TRACKER ──
const preorderProgressIcons = [
...
const readyProgressIcons = [
  { key: 'ordered', label: 'ชำระค่าของ' },
  { key: 'verify', label: 'รอแอดมินตรวจสอบสลิป' },
  { key: 'process', label: 'เตรียมส่ง' },
  { key: 'done', label: 'จัดส่งสำเร็จ' },
]

function getProgressIcons(order) {
  const orderType = String(order?.Order_type || '').trim().toLowerCase()
  return orderType === 'ready' ? readyProgressIcons : preorderProgressIcons
}
...
function getProgressStep(order) {
  ...
  // สำหรับสินค้าพร้อมส่ง (Ready) มี 4 ขั้นตอน
  if (orderType === 'ready') {
    if (['delivered', 'completed'].includes(normalized)) return 4
    if (['shipped', 'ready_to_ship', 'paid'].includes(normalized)) return 3
    if (['slip_submitted'].includes(normalized)) return 2
    return 1
  }

  // สำหรับสินค้าพรีออเดอร์ (Preorder) มี 5 ขั้นตอน
...
<template>
  <div class="order-list-page">
    ...
    <div class="content">
      <!-- ── ORDER-TYPE FILTER ── -->
      <div class="type-tabs" v-if="!loading && !error && orders.length">
        <button
          v-for="tab in orderTypeTabs"
          :key="tab.key"
          type="button"
          class="type-tab"
          :class="{ 'type-tab--active': activeOrderTypeTab === tab.key }"
          @click="setOrderTypeTab(tab)"
        >
          {{ tab.label }}
          <span class="type-tab__count">{{ orderTypeCounts[tab.key] }}</span>
        </button>
      </div>
        ...
        <div class="order-cards">
          ...
          >
            ...
            <div class="progress-track" v-if="getProgressStep(order) > 0">
              <template v-for="(icon, i) in getProgressIcons(order)" :key="icon.key">
                <span
                  class="progress-step"
                ...
                >
                  ...
                  <svg v-else-if="icon.key === 'verify'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                    <polyline points="14 2 14 8 20 8"/>
                    <circle cx="12" cy="14" r="3"/>
                    <line x1="14.12" y1="16.12" x2="16" y2="18"/>
                  </svg>
                ...
                <span
                  v-if="i < getProgressIcons(order).length - 1"
...
.type-tab:hover { color: var(--primary-dark); }
.type-tab--active {
  background: var(--primary);
  color: #fff;
  box-shadow: 0 2px 10px rgba(111, 80, 160, 0.35);
}
.type-tab__count {
  font-size: 0.75rem;
  font-weight: 700;
