...
const fetchOrders = async () => {
  ...
  try {
    loading.value = true
    error.value = null
    const res = await fetch(`http://localhost:3001/api/orders?user_id=${userId}`)
        ...
        <div class="order-cards">
          ...
          >
            ...
            <div class="order-row__head">
              ...
              <span
                class="status-badge"
                :style="{ color: getStatus(order.status).color, background: getStatus(order.status).bg }"
              >
            ...
            <div class="order-row__meta">
              <div class="meta-item">
                ...
                <span class="type-chip" :class="order.Order_type === 'Preorder' ? 'type-chip--pre' : 'type-chip--ready'">
...
.list-count { font-size: 0.88rem; color: #9a7dbf; font-weight: 700; }
...
.order-id-badge {
  ...
  background: #f0e6ff; border-radius: 8px; padding: 3px 12px;
}
.status-badge {
...
.meta-label { font-size: 0.72rem; color: #9a7dbf; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; }
.meta-value { font-size: 0.88rem; font-weight: 600; color: var(--text); }

.type-chip {
  display: inline-block; font-size: 0.82rem; font-weight: 700;
  border-radius: 8px; padding: 2px 10px;
}
.type-chip--ready  { color: #059669; background: #ecfdf5; }
.type-chip--pre    { color: #7c3aed; background: #f5f3ff; }
...
/* ── STATES ── */
.state-wrap { text-align: center; padding: 4rem 1rem; color: #9a7dbf; }
.loader {
...
.empty-text { font-size: 1rem; font-weight: 700; }
.error-box {
  background: #fef2f2; border: 1px solid #fecaca;
  border-radius: 10px; padding: 1rem; color: #b91c1c;
