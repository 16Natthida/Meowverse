...
const API_BASE_URL =
  import.meta.env.VITE_API_BASE_URL || import.meta.env.VITE_API_BASE || 'http://localhost:3001/api'
...
const confirmPayment = async () => {
  ...
  try {
    ...
    if (!order.value.order_id) {
      ...
      slipFormData.append('shipping_name', '-')
      slipFormData.append('shipping_phone', '-')
      slipFormData.append('shipping_address', '-')
      slipFormData.append('notes', '')
        ...
        <div class="order-details">
          <div class="section-card">
            ...
            <div class="preorder-notice" style="background: #ecfdf5; border-color: #10b981" v-else>
              <strong style="color: #10b981">รอบ 2 (ชำระค่านำเข้า):</strong>
              <span style="color: #059669">แอดมินได้แจ้งค่านำเข้าแล้ว กรุณาชำระเงินค่านำเข้า</span>
            </div>
              ...
              <div class="item-list">
                <div v-for="item in readyItems" :key="item.detail_id" class="order-item">
                  ...
                  <div class="item-info">
                    ...
                    <p v-if="item.preorder_round_id" class="item-round">
                      รอบนำเข้า #{{ item.preorder_round_id }}
                    </p>
                    <p class="item-price-small">
                      ฿{{ Number(item.price || item.Price || item.unit_price).toLocaleString() }}
                    </p>
                  </div>
                  ...
                  <div class="item-total">
                    ฿{{
                      (
                        Number(item.price || item.Price || item.unit_price) * Number(item.qty || 0)
                      ).toLocaleString()
                    }}
                  </div>
                </div>
        ...
        <div class="payment-section">
          ...
          <div class="section-card summary-card">
            ...
            <div class="summary-row total">
              ...
              <span class="total-amount" v-if="importFeeTotal > 0"
                >฿{{ importFeeTotal.toLocaleString() }}</span
              >
...
.navbar {
  ...
  height: 60px;
  background: #fff;
  border-bottom: 1px solid var(--border, #eadff5);
  position: sticky;
  top: 0;
  z-index: 100;
}
.back-btn {
  ...
  border-radius: 20px;
  padding: 5px 12px;
  border: 1px solid var(--border, #eadff5);
  background: #fff;
  cursor: pointer;
  color: var(--primary, #6f50a0);
  font-weight: bold;
  font-family: inherit;
}
.back-btn:hover {
...
.notice {
  ...
  border-left: 4px solid var(--primary, #6f50a0);
  font-size: 0.9rem;
  font-weight: 600;
}
.notice--error {
...
.payment-method {
  ...
  gap: 12px;
  padding: 12px 14px;
  border: 2px solid var(--border, #eadff5);
  border-radius: var(--radius-sm, 10px);
  cursor: pointer;
  transition: all 0.2s;
  background: #fff;
...
.payment-method:hover {
  border-color: var(--primary, #6f50a0);
}
...
/* Upload */
.upload-area {
  border: 2px dashed var(--border, #eadff5);
  border-radius: var(--radius-sm, 10px);
  overflow: hidden;
  transition: all 0.2s;
}
...
.divider {
  border: none;
  border-top: 1px dashed var(--border, #eadff5);
  margin: 10px 0;
}
.pending-text {
  color: #f59e0b;
...
.loader {
  ...
  border-top: 3px solid var(--primary, #6f50a0);
  border-radius: 50%;
  width: 32px;
  height: 32px;
