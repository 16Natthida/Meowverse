...
const amountDue = computed(() => {
  ...
  // ใช้ itemsSubtotal (คำนวณจาก order_details สดๆ) ก่อนเสมอ เพราะ order.total_amount ที่ backend
  // แคชไว้อาจไม่ถูกอัปเดตหลังแยกออเดอร์ (รายการที่ถูกแยกไปจะมี qty = 0 ใน order_details แล้ว
  // แต่ total_amount เดิมอาจยังรวมยอดของรายการนั้นค้างอยู่) จึง fallback ไปใช้ total_amount
  // เฉพาะตอนที่ยังไม่มีข้อมูล items เท่านั้น
  if (['paid', 'ready to ship', 'shipped', 'delivered'].includes(status)) {
    const productTotal = itemsSubtotal.value > 0 ? itemsSubtotal.value : Number(order.value?.total_amount || 0)
    return productTotal + importFeeTotal.value + shippingFee
  }

  if (isFullyPaid.value) return 0

  if (isDelayedOrder.value && importFeeTotal.value <= 0) {
    return 0
  }
          ...
          <div class="payment-sticky">
            ...
            >
              ...
              <div class="summary-header">
               ...
               <div v-if="normalizedOrderStatus !== 'delayed'" class="terms-box">
