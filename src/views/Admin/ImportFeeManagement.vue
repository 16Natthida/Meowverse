  ...
  <div class="import-fee-page">
    ...
    <div v-if="selectedRound && statusGroups.length > 0" class="card table-card">
        ...
        <div v-for="group in statusGroups" :key="group.status" class="status-group">
          ...
          <tbody>
            ...
            >
              ...
              <td class="num">
                <div v-if="item.isFeeLocked" class="fee-locked-chip">
                  <svg
                    width="14"
                    height="14"
                    viewBox="0 0 24 24"
                  ...
                  {{ formatMoney(item.current_import_fee) }}
                </div>
      ...
      <div class="note-warning">
        ...
        ค่านำเข้าที่กรอกเป็นยอดรวมของสินค้าประเภทนี้ทั้งรอบ ระบบจะแบ่งตามจำนวนที่สั่งรวม
      </div>
...
const groupedProducts = computed(() => {
  ...
  return selectedRound.value.products.map((item) => {
    ...
    return {
      ...
      // รายการที่มี order_details.Import_fee บันทึกไว้แล้ว (ไม่ใช่ 0.00) ให้ล็อกถาวร
      // ห้ามแก้ไข/บันทึกทับค่าเดิมอีก
      isFeeLocked: Number(item.current_import_fee) > 0,
      canEnterImportFee:
        arrivalStatus === 'Arrived' && receivedQty >= orderedQty && !(Number(item.current_import_fee) > 0),
    }
  })
})
...
async function saveImportFees() {
  ...
  for (const item of groupedProducts.value) {
    if (item.isFeeLocked) {
      // รายการที่มี order_details.Import_fee บันทึกไว้แล้ว (ไม่ใช่ 0.00):
      // ต้อง "ส่งค่าเดิมกลับไปเสมอ" ห้ามข้ามรายการนี้ไปเฉยๆ เพราะ backend จะถือว่า
      // รายการที่ไม่ถูกส่งมาในลิสต์นี้ = ให้เคลียร์ค่าทิ้ง (กลายเป็น 0.00) ทำให้ข้อมูลเดิมหาย
      // ดังนั้นจึงยังคงส่ง key/prod_id/flavor ไปแต่ล็อกค่า import_fee ไว้เท่าค่าเดิมเป๊ะๆ
      // เพื่อไม่ให้ค่าที่กรอกไว้แล้วถูกบันทึกทับกลายเป็น 0.00
      fees.push({
        key: item.key,
        prod_id: item.prod_id,
        flavor: item.flavor || null,
        import_fee: item.current_import_fee,
      })
      continue
    }
...
.fee-table tbody tr.row--locked {
  background: #f3f4f6;
}
.fee-table tbody tr.row--locked:hover {
  background: #f3f4f6;
}
.fee-table tbody tr.row--locked .cell-product,
.fee-table tbody tr.row--locked .cell-price {
  color: #9ca3af;
}
.fee-locked-chip {
  display: inline-flex;
