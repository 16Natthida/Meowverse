  ...
  <section class="admin-page">
    ...
    <section v-if="productPanelOpen" class="form-panel">
      ...
      <form class="product-form" @submit.prevent="submitForm">
        ...
        <!-- ── READY TO SHIP TOGGLE CARD ── -->
        <div class="ready-ship-toggle-card">
          <div class="ready-ship-toggle-card__info">
            <div class="ready-ship-toggle-card__title">
              <span class="icon">📦</span>
              <span>สถานะสินค้าพร้อมส่ง</span>
              <span class="status-badge" :class="{ 'status-badge--active': form.readyToShipEnabled }">
                {{ form.readyToShipEnabled ? 'เปิดพร้อมส่ง' : 'ปิดอยู่' }}
              </span>
            </div>
            <p class="ready-ship-toggle-card__desc">เปิดให้ลูกค้าสั่งซื้อแบบ "พร้อมส่ง" ได้ทันทีหลังจากมีสต็อก</p>
          </div>
...
.ready-ship-toggle-card {
  ...
  border: 1px solid #eadff5;
  border-radius: 14px;
  box-shadow: 0 4px 14px rgba(111, 80, 160, 0.05);
  transition: all 0.2s ease;
}

.ready-ship-toggle-card:hover {
  border-color: #d8c4f0;
  box-shadow: 0 6px 18px rgba(111, 80, 160, 0.09);
}

.ready-ship-toggle-card__info {
  display: flex;
  flex-direction: column;
  gap: 0.25rem;
}

.ready-ship-toggle-card__title {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  font-size: 0.9rem;
...
.ready-ship-toggle-card__title .icon {
  font-size: 1.05rem;
}
...
.switch-control input:checked + .switch-slider {
  background: linear-gradient(135deg, #cda2fb 0%, #b684ed 100%);
  box-shadow: 0 2px 8px rgba(182, 132, 237, 0.4);
}
