...
app.get('/api/admin/inventory-intake/rounds', authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const [intakeHistoryRows] = await pool.query(
      `SELECT od.preorder_round_id AS round_id,
              COUNT(DISTINCT i.intake_id) AS intake_count,
              MAX(i.created_at) AS last_intake_at
       FROM inventory_intake_sessions i
       JOIN inventory_intake_session_items isi ON isi.intake_id = i.intake_id
       JOIN order_details od ON od.detail_id = isi.detail_id
       WHERE od.preorder_round_id IS NOT NULL
       GROUP BY od.preorder_round_id`,
    )
    const intakeHistoryMap = new Map(
      intakeHistoryRows.map((row) => [Number(row.round_id), row]),
    )
    ...
    for (const roundRow of roundRows) {
      ...
      roundMap.set(roundId, {
        ...
        intake_confirmed: Number(intakeHistoryMap.get(roundId)?.intake_count || 0) > 0,
        last_intake_at: intakeHistoryMap.get(roundId)?.last_intake_at || null,
        ordersMap: new Map(),
        itemsMap: new Map(),
      })
    }
    ...
    for (const row of rows) {
      ...
      if (String(row.arrival_status || '').toLowerCase() !== 'arrived') {
        item.has_editable_details = true
      }
    ...
    const rounds = Array.from(roundMap.values()).map((round) => {
      ...
      return {
        ...
        intake_confirmed: round.intake_confirmed,
        last_intake_at: round.last_intake_at,
        orders,
        items,
        summary: {
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const [lineKey, lineDetails] of lineGroups.entries()) {
        ...
        const arrivedDetails = lineDetails.filter(
          (detail) => String(detail.arrival_status || '').toLowerCase() === 'arrived',
        )
        const immutableArrivedQty = arrivedDetails.reduce(
          (sum, detail) =>
            sum + Math.min(Math.max(0, Number(detail.received_qty) || 0), Math.max(0, Number(detail.ordered_qty) || 0)),
          0,
        )
        // Arrived quantities are final. A correction may only change the
        // remaining non-arrived details, never reduce an Arrived detail.
        const targetReceived = Math.min(
          Math.max(requestedReceived, immutableArrivedQty),
          totalOrderedForLine,
        )
        const finalReceivedByDetail = new Map()
        let remainingExisting = Math.max(targetReceived - immutableArrivedQty, 0)

        for (const detail of arrivedDetails) {
          finalReceivedByDetail.set(
            detail.detail_id,
            Math.min(Math.max(0, Number(detail.received_qty) || 0), Math.max(0, Number(detail.ordered_qty) || 0)),
          )
        }

        const editableDetails = lineDetails.filter(
          (detail) => String(detail.arrival_status || '').toLowerCase() !== 'arrived',
        )

        // Preserve existing quantities for editable details before allocating
        // any newly received quantity in stable order.
  ...
  try {
    ...
    res.json(
      rows.map((row) => ({
        ...
        import_fee_total: Number(row.import_fee_total) || 0,
        china_shipping_total_thb: Number(row.china_shipping_total_thb) || 0,
