...
app.delete('/api/preorder-rounds/:id', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const [result] = await pool.query(
      'DELETE FROM preorder_rounds WHERE round_id = ?',
      [Number(req.params.id)]
    )
...
app.delete('/api/preorder-rounds/:id/products/:productId', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const [result] = await pool.query(`
...
app.put('/api/preorder-rounds/:id/products/:productId', authenticateToken, requireAdmin, async (req, res) => {
  const roundId = Number(req.params.id)
