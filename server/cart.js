  ...
  try {
    ...
    try {
      ...
      if (existing.length > 0) {
        ...
        if (requestedType === 'ready-to-ship' && newQty > Number(product.stock || 0)) {
      ...
      const [result] = await connection.query(
        ...
        [user_id, prod_id, Number(qty), requestedType, effectiveFlavor || null, effectiveRoundId, requestedType === 'preorder' ? effectiveRoundId : null, effectiveRoundPrice],
      )
